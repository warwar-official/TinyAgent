...
def get_current_weather(location: str) -> dict:
    ...
    location_json = request("GET", f"https://nominatim.openstreetmap.org/search?q={location_url}&format=json").json()
    if location_json:
        lat = location_json[0]["lat"]
        lon = location_json[0]["lon"]
        weather_json = request("GET", f"https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}&current_weather=true").json()
        if weather_json:
            answer = {
