...
class WorkerRole(AIRole):
    ...
    def run(self, payload: dict) -> dict:
        ...
        Worker does NOT have access to: identity, history, memory.
        ...
        user_prompt = f"Current step: {json.dumps(current_step, ensure_ascii=False)}\n"
        user_prompt += f"Available tools:\n{tools_text}\n"
        if abilities:
            user_prompt += f"Your abilities:\n{json.dumps(abilities, ensure_ascii=False)}\n"
        if tool_results:
            user_prompt += f"Previous intermediate results: {json.dumps(tool_results, ensure_ascii=False)}\n"
        if feedback:
